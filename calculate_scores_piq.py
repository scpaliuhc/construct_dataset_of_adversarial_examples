...
Version          :1.0
'''
# /data0/lhc/construct_dataset_of_adversarial_examples
...
class AEData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        ref_img = Image.open(os.path.join(
            self.ref_root, self.ref_names[index]))
        ...
        try:
            for file in sel_files:
                ae_img = Image.open(os.path.join(
                    os.path.join(self.ae_root, self.ae_mth, file)))
                ae_img = self.trans(ae_img)
                ae_imgs.append(ae_img)
        except:
            print(
                f"An error happened on {file}, the corresponding ref file {self.ref_names[index]}")
            exit()
...
def calculate_score_(names, refs, aes, cols):
    ...
    for func in cols:
        ...
        try:
            ...
            if useGPU:
                tmp_dic[metr] = eval('piq.'+func)(refs, aes,
...
def calculate_score(loader, csv_file, batch_size, cols, mth, init):
    ...
    for id, (ref, aes, ref_name, ae_names) in enumerate(loader):
        ...
        for i in range(0, ae_count, batch_size):
            tmp_dic = calculate_score_(ae_names[i:(
                i+1)*batch_size], refs[i:(i+1)*batch_size], aes[i:(i+1)*batch_size], cols)
...
def main(args):
    ...
    funcs = ['ssim', 'ms_ssim', 'psnr', 'vif_p', 'vsi',
             'fsim', 'gmsd', 'ms_gmsd', 'haarpsi', 'mdsi']
...
if __name__ == '__main__':
    ...
    parser.add_argument("--method", default='PGD', type=str, choices=['FGSM', 'BIM', 'PGD', 'CW', 'DeepFool', 'OnePixel',
                        'Square', 'SparseFool', 'Boundary', 'SimBA', 'AdvPatch', 'GAP', 'NES', 'Daedalus', 'hnmPGD'], help="the attack method.")
    parser.add_argument("--ref", type=str,
                        default='/data0/lhc/dataset/IQA_AE/REF/ref', help="")
    parser.add_argument(
        "--adv", type=str, default='/data0/lhc/construct_dataset_of_adversarial_examples/generated', help="")
